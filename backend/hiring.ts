...
export const hiringStore:Store={
  async read<T>(key:string){const {items}=await db.list<T>(prefix+key,{limit:2});if(items.length>1)throw Error('Duplicate storage partition; operator reconciliation required: '+key);rowIds.set(key,items[0]?.id||null);return items[0] as T||null;},
  async write(key:string,record:Record<string,unknown>){
    ...
    if(!rowIds.has(key))await hiringStore.read(key);const rowId=rowIds.get(key);
    if(rowId){const [ok]=await db.update(prefix+key,[{id:rowId,record:clean}]);if(!ok)throw Error('Hiring storage update failed: '+key);}
    else{const [id]=await db.add(prefix+key,[clean]);if(!id)throw Error('Hiring storage insert failed: '+key);rowIds.set(key,id);}
  }
};
export async function migrateHiring(store=hiringStore){
  ...
  if(!version)await store.write('schema',{version:1,applied_at:new Date().toISOString(),timezone:'Asia/Tokyo',timestamp_format:'UTC ISO8601',migration:'001-additive-hiring-history'});
}
...
export async function runHiring(now:string,store=hiringStore,collector=collectSource){
  await migrateHiring(store);const day=jstDay(now);const summaries:Summary[]=[];const failures:string[]=[];
  // Each source has durable acquisition, ingestion, commit and aggregate checkpoints.
  for(const source of hiringSources){try{
    let summary=await store.read<Summary>('commit:'+source.id+':'+day);
    if(!summary){const acquisitionKey='acquisition:'+source.id+':'+day;
      let saved=await store.read<{result:Result;observed_at:string}>(acquisitionKey);
      if(!saved){saved={result:await collector(source),observed_at:new Date().toISOString()};
    ...
    const key='month:'+day.slice(0,7)+':'+(hiringSources.indexOf(source)%4);const month=await store.read<Month>(key)||{days:{}};
    month.days[day]={...month.days[day],[source.id]:summary};await store.write(key,month);
    console.log(JSON.stringify({event:'hiring_source',source:source.id,day,status:summary.status,observed:summary.observed}));
  }catch(e){const message=e instanceof Error?e.message:'Unknown storage failure';failures.push(source.id+': '+message);
...
export async function hiringHistory(query:Record<string,string>,store=hiringStore){
  const today=jstDay(new Date().toISOString());const day=query.date||today;const year=query.year||day.slice(0,4);
  ...
  const months:Month[]=[];for(const month of monthKeys){for(let shard=0;shard<4;shard++)months.push(await store.read<Month>('month:'+month+':'+shard)||{days:{}});}
  const joined:Month={days:{}};for(const month of months)for(const [date,sources]of Object.entries(month.days))joined.days[date]={...joined.days[date],...sources};
  const all=Object.entries(joined.days).map(([date,sources])=>({date,sources:Object.values(sources).filter(s=>!query.company||s.company_id===query.company)})).sort((a,b)=>a.date.localeCompare(b.date));
  const trend=all.map(d=>{const known=d.sources.filter(s=>s.observed!==null);return {date:d.date,observed:known.length?known.reduce((n,s)=>n+s.observed!,0):null,confirmed_sources:known.length,complete_sources:known.filter(s=>s.complete).length,first_observed:known.reduce((n,s)=>n+s.first_observed,0),categories:Object.fromEntries(categories.map(c=>[c,known.length?known.reduce((n,s)=>n+(s.categories[c]||0),0):null]))};});
  ...
  const sources=hiringSources.filter(s=>!query.company||s.company_id===query.company).map(s=>{const summary=selected.find(x=>x.source===s.id);return {id:s.id,company_id:s.company_id,company:s.company,url:s.url,summary:summary||null,stale:!summary||Date.now()-Date.parse(summary.checked_at)>30*3600000};});
  const actualDay=query.date?day:latest?.day||day;let jobs:Job[]=[];let nextPage:number|null=null;
  const page=Number(query.page||0);if(!Number.isInteger(page)||page<0||page>15)throw Error('Invalid page');
  if(query.company){const src=hiringSources.find(s=>s.company_id===query.company)!;const commit=await store.read<Summary>('commit:'+src.id+':'+actualDay);
    if(commit){jobs=(await store.read<{jobs:Job[]}>('observations:'+src.id+':'+actualDay+':'+page))?.jobs||[];if((page+1)*25<(commit.observed||0))nextPage=page+1;}
  }
  ...
  if(query.job){if(!/^[a-f0-9]{64}$/.test(query.job))throw Error('Invalid job');currentJob=await store.read<Job>('job:'+query.job);if(currentJob&&query.company&&currentJob.company_id!==query.company)throw Error('Invalid job company');}
