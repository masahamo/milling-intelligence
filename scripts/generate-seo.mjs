...
const details=[
 ...companyIds.map(id=>{const name=String(nameById.get(id)||id).replace(/^株式会社|株式会社$/g,'').trim(),millId=millAliases[id]||id,hasMill=millSet.has(millId),hasCareer=careerSet.has(id);return {path:'/company/'+encodeURIComponent(id),title:name+'｜製粉事業・工場・製粉能力・採用情報',description:clip(name+'の会社概要、製粉事業、工場、確認済み製粉能力、採用・転職情報、関連ニュース、公式情報源を既存の公開データから確認できます。'),h1:name+'｜製粉事業・工場・製粉能力・採用情報',body:name+'の製粉事業、工場、製粉能力、採用情報を会社IDで統合しています。未確認情報は推定しません。',links:[['/companies','会社一覧'],...(hasMill?[[`/mills/${millId}`,'工場を見る'],['/compare','ランキングを見る']]:[]),...(hasCareer?[[`/career/${id}`,'採用を見る']]:[])]}}),
 ...careerIds.map(id=>({path:'/career/'+encodeURIComponent(id),title:'製粉会社の採用・転職情報',description:'製粉会社の公開採用情報、募集職種、勤務地、求人履歴を確認できます。',h1:'製粉会社の採用・転職情報',body:'公開採用情報、募集職種、勤務地、求人履歴を会社単位で確認できます。',links:[['/career','Career'],['/companies','Companies']]})),
